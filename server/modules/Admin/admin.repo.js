...
exports.create = async (formObject) => {
    try {
        ...
        await resultObject.populate({ path: "permission", select: "name permissions" });
        return {
            success: true,
            code: 201,
            result: resultObject,
...
exports.update = async (_id, formObject) => {
    try {
        ...
        const resultObject = await adminModel.findByIdAndUpdate({ _id }, formObject, { new: true, select: "-password", populate: { path: "permission", select: "name permissions" } })
...
exports.remove = async (_id) => {
    try {
        const resultObject = await adminModel.findByIdAndUpdate({ _id }, { isActive: false })
