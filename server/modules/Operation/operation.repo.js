...
const recordRepo = require('../Record/record.repo');
...
exports.create = async (formObject) => {
    try {
        ...
        await this.manageRecord(resultObject)
        return {
            success: true,
            code: 201,
            result: resultObject,
...
exports.manageRecord = async (operation) => {
    let batteryHoursPercentage, distances, accuracyPercentage, count;
    const startDate = new Date(operation.startDate)
    const month = startDate.toLocaleString('en-US', { month: 'short' });
    let record = await recordRepo.find({ robot: operation.robot, month: month })
    if (record.success) {
        count = record.result.count + 1;
        batteryHoursPercentage = (record.result.batteryHoursPercentage + operation.batteryHours) / count;
        console.log(batteryHoursPercentage);
        distances = record.result.distances + operation.distance;
        accuracyPercentage = (record.result.accuracyPercentage + operation.accuracy) / count;
        record = await recordRepo.update(record.result._id, {
            batteryHoursPercentage: batteryHoursPercentage,
            distances: distances,
        ...
        count = 1;
        batteryHoursPercentage = operation.batteryHours / count;
        distances = operation.distance;
        accuracyPercentage = operation.accuracy / count;
        record = await recordRepo.create({
            robot: operation.robot,
            month: month,
            batteryHoursPercentage: batteryHoursPercentage,
            distances: distances,
            accuracyPercentage: accuracyPercentage,
