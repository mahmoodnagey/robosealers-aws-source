...
exports.createOperation = async (req, res) => {
    try {
        if (!req.body.user || req.body.user !== req.tokenData._id) return res.status(400).json({
            success: false,
            code: 400,
...
exports.listOperations = async (req, res) => {
    try {
        const filterObject = { ...req.query, user: req.tokenData._id };
...
exports.uploadImages = async (req, res) => {
    try {
        ...
        const existingObject = await operationRepo.find({ _id: req.query._id, user: req.tokenData._id })
